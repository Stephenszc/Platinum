...
basefile = pd.read_csv(r'72featWithLabels.csv')
output = []
for i in range(1,8795):
    ...
    for j in range(i,8795):
        smoteOut = []
        if i != j & (basefile.iloc[i,72] is basefile.iloc[j,72]):
            ...
            smoteOut.append(basefile.iloc[j,72]) #Adds protein's type name to last column of 'smotted' values 
